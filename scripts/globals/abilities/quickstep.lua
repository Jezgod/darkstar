        ...
        if mjob == dsp.job.DNC then
            if (target:hasStatusEffect(dsp.effect.LETHARGIC_DAZE_1)) then
                ...
                if (player:hasStatusEffect(dsp.effect.PRESTO)) then
                    target:addStatusEffect(dsp.effect.LETHARGIC_DAZE_3,1,0,duration+30)
                    daze = 3
                    effect = 3
                else
                    target:addStatusEffect(dsp.effect.LETHARGIC_DAZE_2,1,0,duration+30)
                    daze = 2
                    effect = 2
                end

            elseif (target:hasStatusEffect(dsp.effect.LETHARGIC_DAZE_2)) then
                local duration = target:getStatusEffect(dsp.effect.LETHARGIC_DAZE_2):getDuration()
                target:delStatusEffectSilent(dsp.effect.LETHARGIC_DAZE_2)
                if (player:hasStatusEffect(dsp.effect.PRESTO)) then
                    target:addStatusEffect(dsp.effect.LETHARGIC_DAZE_4,1,0,duration+30)
                    daze = 3
                    effect = 4
                else
                    target:addStatusEffect(dsp.effect.LETHARGIC_DAZE_3,1,0,duration+30)
            ...
            elseif (target:hasStatusEffect(dsp.effect.LETHARGIC_DAZE_3)) then
                local duration = target:getStatusEffect(dsp.effect.LETHARGIC_DAZE_3):getDuration()
                target:delStatusEffectSilent(dsp.effect.LETHARGIC_DAZE_3)
                if (player:hasStatusEffect(dsp.effect.PRESTO)) then
                    target:addStatusEffect(dsp.effect.LETHARGIC_DAZE_5,1,0,duration+30)
                    daze = 3
                    effect = 5
                else
                    target:addStatusEffect(dsp.effect.LETHARGIC_DAZE_4,1,0,duration+30)
            ...
            elseif (target:hasStatusEffect(dsp.effect.LETHARGIC_DAZE_4)) then
                local duration = target:getStatusEffect(dsp.effect.LETHARGIC_DAZE_4):getDuration()
                target:delStatusEffectSilent(dsp.effect.LETHARGIC_DAZE_4)
                if (player:hasStatusEffect(dsp.effect.PRESTO)) then
                    daze = 3
                else
                    daze = 2
            ...
            elseif (target:hasStatusEffect(dsp.effect.LETHARGIC_DAZE_5)) then
                local duration = target:getStatusEffect(dsp.effect.LETHARGIC_DAZE_5):getDuration()
                target:delStatusEffectSilent(dsp.effect.LETHARGIC_DAZE_5)
                target:addStatusEffect(dsp.effect.LETHARGIC_DAZE_5,1,0,duration+30)
                daze = 1
                effect = 5
                ...
                if (player:hasStatusEffect(dsp.effect.PRESTO)) then
                    target:addStatusEffect(dsp.effect.LETHARGIC_DAZE_2,1,0,60)
                    daze = 3
                    effect = 2
                else
                    target:addStatusEffect(dsp.effect.LETHARGIC_DAZE_1,1,0,60)
                    daze = 2
                    effect = 1
                end
            end
