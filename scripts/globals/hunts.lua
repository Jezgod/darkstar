...
function dsp.hunts.onTrigger(player,npc,event)
...
function dsp.hunts.onEventUpdate(player,csid,option)
...
dsp.hunts.clearHuntVars = function(player)
    player:setCharVar("[hunt]id", 0)
    player:setCharVar("[hunt]status", 0)
end

function dsp.hunts.onEventFinish(player, csid, option)
    ...
    elseif option == 5 then
        ...
        dsp.hunts.clearHuntVars(player)
...
function dsp.hunts.checkHunt(mob, player, mobHuntID)
    ...
    if player:getCharVar("[hunt]status") == 1 and playerHuntID == mobHuntID then
        player:messageBasic(dsp.msg.basic.FOV_DEFEATED_TARGET + 20)
