...
function onTrigger(player, npc)
    ...
    local transformations = player:getQuestStatus(AHT_URHGAN, dsp.quest.id.ahtUrhgan.TRANSFORMATIONS)
    ...
    elseif anEmptyVessel == QUEST_COMPLETED and beginnings == QUEST_AVAILABLE and player:getCurrentMission(TOAU) > dsp.mission.id.toau.IMMORTAL_SENTRIES
            and currentJob == dsp.job.BLU and player:getMainLvl() >= AF1_QUEST_LEVEL then
        if divinationReady then
    ...
    -- OMENS
    elseif beginnings == QUEST_COMPLETED and omens == QUEST_AVAILABLE and currentJob == dsp.job.BLU and player:getMainLvl() >= AF2_QUEST_LEVEL then
        if divinationReady then
            if waoudNeedToZone == 1 then
                player:startEvent(78, player:getGil()) -- dummy questions, costs you 1000 gil
            else
    ...
    -- TRANSFORMATIONS
    elseif omens == QUEST_COMPLETED and transformations == QUEST_AVAILABLE and currentJob == dsp.job.BLU then
        if divinationReady then
            if waoudNeedToZone == 1 then
                player:startEvent(78, player:getGil()) -- dummy questions, costs you 1000 gil
